...
function typeBadgeClasses(t: DexPoolRow["type"]) {
  ...
  if (t === "CONCENTRATED") return "bg-cyan-500/10 text-cyan-300 border-cyan-500/20";
...
export function PoolsTable({
  ...
  return (
    <Surface variant="table" className="flex flex-col">
            ...
            {loading ? (
              ...
              rows.map((r) => (
                <tr key={r.id} className="group hover:bg-slate-800/60 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div className="min-w-5 h-5 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-[8px] text-slate-200 px-1">
                        {r.token0Symbol.slice(0, 4)}
                      </div>
